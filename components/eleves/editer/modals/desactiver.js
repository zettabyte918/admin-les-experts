...
function DesactivaterEleveModal({ id, open, setOpen }) {
  const { deactivateStudentById } = useApi();

  const cancelButtonRef = useRef(null);

  const handelDeactivateEleve = async () => {
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Voulez-vous vraiment désactiver ce compte? Toutes ses
                        données seront désactivées de nos serveurs. Cette action
                        peut être annulée.
                      </p>
                    </div>
                  </div>
